...
$(function($) {
	...
	MBox.prototype = {
		...
		build: function() {
			...
			}).append('<span class="font_size_close">&times;</span>').prependTo(container);
		...
		createNavigation: function() {
			...
			if(this.imagesCount > 1) {
				...
				navigationCount.append(' de ' + this.imagesCount); // Add to the end.
				...
				if(this.imagesCount > this.settings.imagesPerPage) {
					...
					for(var i = 0; i < totalPages; i++) {
						var circ = $('<i/>', {
							'class': 'fas fa-circle point gallery_thumbnail',
							'data-page-number': i
